...
Helm chart for deploying **nodecore** to Kubernetes.

> **Note**: Helm 3.8.0 or later is required for OCI registry support.

The chart source lives in [`chart/nodecore`](./chart/nodecore) in this repository
...
```bash
helm upgrade --install nodecore   oci://ghcr.io/drpcorg/nodecore-chart   --version <VERSION>   --namespace nodecore   --create-namespace
```
...
- Helm v3.8.0+
- A default `StorageClass` if you enable persistence
...
```yaml
image:
  repository: drpcorg/nodecore:latest
  tag: "latest"
...
```bash
helm upgrade --install nodecore   oci://ghcr.io/drpcorg/nodecore-chart   --version <VERSION>   --namespace nodecore   --create-namespace   -f values.yaml
```
...
This removes all Kubernetes resources created by the chart. PersistentVolumes
created by a dynamic `StorageClass` might need manual cleanup, depending on the
`reclaimPolicy` of your storage class.
...
[`values.yaml`](chart/nodecore/values.yaml).
...
See [`values.yaml`](chart/nodecore/values.yaml) for the full list of options and their
defaults.
