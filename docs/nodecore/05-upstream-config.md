...
| Lower-bound detector (TON) | TON | `disable-lower-bounds-detection` | v2: archival probe via `lookupBlock` for masterchain seqno 1; v3: reads the indexer's history floor from `masterchainInfo` `first.seqno`, which may legally move *down* when the index is backfilled deeper |
