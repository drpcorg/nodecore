...
| Cosmos chain validator (gRPC) | Cosmos (`grpc`) | `disable-chain-validation`, `disable-settings-validation`, `disable-validation` | The same check over the SDK gRPC API: `cosmos.base.tendermint.v1beta1.Service/GetNodeInfo` → `default_node_info.network` |
...
| Client label detector (Cosmos gRPC) | Cosmos (`grpc`) | `disable-labels-detection` | Publishes `client_type=cosmos` and the **SDK application** version from `cosmos.base.tendermint.v1beta1.Service/GetNodeInfo`, with the same CometBFT-version fallback as the LCD detector |
