...
- `grpc` - native gRPC endpoints (declared by spec on a per-chain basis; currently Sui's `sui.rpc.v2`). The URL scheme selects transport security: `grpc://host:port` is plaintext, anything else (`grpcs://`, `https://`) is TLS with the system roots or the connector's `ca`. `headers` are sent as per-call gRPC metadata; `response-header-deny` filters response *metadata* (the reserved `grpc-*` family and `content-type` are always stripped). One long-lived HTTP/2 connection per connector with client keepalives; every call is capped at 60s (a caller's own tighter deadline always wins and propagates to the node as `grpc-timeout`). For gRPC-native chains this can be the upstream's **only** connector — it then drives head, health, chain validation, labels and lower bounds. Unary methods only in this version; server-streaming is a follow-up. Client-facing gRPC traffic enters through the [gRPC chain ingress](14-grpc-ingress.md)
