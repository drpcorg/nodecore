...
* `<chain>.validate-lag` - When enabled, derives each upstream's availability from how far its head trails the chain head. An upstream that is otherwise `Available` but lags behind the best observed head by more than the chain's `settings.lags.syncing` threshold (a chain-metadata value from the embedded `chains.yaml`, overridable via [`NODECORE_EXTRA_CHAINS_PATH`](#extending-the-chain-registry-at-startup)) is marked `Syncing`, which deprioritizes it during routing until it catches up; when the lag drops back within the threshold the upstream's probe-reported status is restored. Unlike `validate-syncing`, which asks each node about its own sync state, this compares heads *across* upstreams of the chain, so it catches nodes that report healthy but silently fall behind. Mode-dependent default: `false` in `default` mode, `true` in `strict` mode
