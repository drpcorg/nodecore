...
- `head-mode` - `subscribe` | `poll`, **_Default_**: `subscribe`. Only consulted when `head-connector` is `grpc` (every other connector type implies its mode: `websocket` subscribes, `json-rpc`/`rest`/`tendermint` poll). `subscribe` opens the chain's head stream (Sui: `SubscriptionService/SubscribeCheckpoints`); `poll` calls the head probe every `poll-interval`. If the chain has no head subscription at all, nodecore logs a warning and polls. If the *node* rejects the stream (e.g. a Sui fullnode without `SubscriptionService` enabled answers `UNIMPLEMENTED`), the head fails - set `head-mode: poll` for such nodes
