...
- **`NativeSubscribe(NativeSubscribeRequest) → stream NativeSubscribeReplyItem`**
  ...
  - **JSON-RPC subscriptions** (`newHeads`, `logs` on EVM; `slotSubscribe`, `accountSubscribe` on Solana; `header.Subscribe`, `blob.Subscribe` on Celestia; any native sub method). `payload` is the JSON params. The subscription is fulfilled through nodecore's shared-subscription engine, so multiple clients asking for the same subscription are coalesced onto a single upstream WebSocket. Each event is a reply item whose `payload` is the bare event result: for Celestia that is the value of the node's `xrpc.ch.val` notification (an `ExtendedHeader` for `header.Subscribe`, a `{Blobs, Height, Header}` object per height for `blob.Subscribe`), without channel id or envelope; the client speaks the go-jsonrpc channel dialect to its own consumers if it needs to. A failure ends the stream with a gRPC status, including the node closing the channel (`xrpc.ch.close`).
