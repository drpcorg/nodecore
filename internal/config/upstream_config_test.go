...
// rest-indexer is a PLAIN connector type (a self-contained indexer API next
// to the node API, e.g. the TON v3 indexer) - unlike rest-additional it may
// be an upstream's only connector and then drives head duty.
func TestOnlyRestIndexerConnectorThenOk(t *testing.T) {
	t.Setenv(config.ConfigPathVar, "configs/upstreams/ton-only-rest-indexer.yaml")
	cfg, err := config.NewAppConfig()
	assert.NoError(t, err, "an upstream may consist of a single rest-indexer connector")
	assert.Equal(t, specs.RestIndexer.String(), cfg.UpstreamConfig.Upstreams[0].HeadConnector,
		"the sole rest-indexer connector must default to head duty")
}
