...
func TestChainSupervisorCreatesNodeGroupsPerClientTypeAndMethods(t *testing.T) {
	...
	// heads are seeded from the state snapshots
	assert.Equal(t, uint64(100), groups[gethId].HeadData.Head.Height)
	assert.Equal(t, uint64(90), groups[erigonId].HeadData.Head.Height)
...
func TestChainSupervisorDropsGroupOnUpstreamRemoval(t *testing.T) {
	...
	chainSupervisor.PublishUpstreamEvent(protocol.UpstreamEvent{Id: "up-1", EventType: &protocol.RemoveUpstreamEvent{}})
