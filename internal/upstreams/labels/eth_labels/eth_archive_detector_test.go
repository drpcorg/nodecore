...
func TestEthArchiveLabelsDetectorDetectLabelsReturnsArchiveLabel(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedBlock: eth_labels.EarliestBlock,
		},
		{
			name:          "uses arbitrum nitro block",
			chain:         chains.ARBITRUM,
			expectedBlock: eth_labels.ArbitrumNitroBlock,
		},
		{
			name:          "uses optimism bedrock block",
			chain:         chains.OPTIMISM,
			expectedBlock: eth_labels.OptimismBedrockBlock,
		},
		{
...
func TestEthArchiveLabelsDetectorDetectLabelsReturnsFalseWhenConnectorReturnsError(t *testing.T) {
	request, err := protocol.NewInternalUpstreamJsonRpcRequest(
		"eth_getBalance",
		[]any{"0x0000000000000000000000000000000000000000", eth_labels.EarliestBlock},
		chains.ETHEREUM,
	)
	require.NoError(t, err)
	...
	detector := eth_labels.NewEthArchiveLabelsDetector("upstream-id", chains.ETHEREUM, time.Second, connector)
