...
// lower proof bound. An upstream that rejects the method as unknown, or answers with an
// unparseable body, is remembered as unsupported and re-asked hourly; transient failures
...
// detectFromProofsSyncStatus resolves the proof lower bound straight from the upstream's
// historical proof store: one call replaces the eth_getProof binary search. The window's
// upper edge is not published - measured op-reth stores keep it at the head.
