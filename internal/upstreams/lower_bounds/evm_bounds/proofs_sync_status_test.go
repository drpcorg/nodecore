...
func TestProofsSyncStatusYieldsProofLowerBound(t *testing.T) {
...
func TestProofsSyncStatusEmptyWindowFallsBackAndRetries(t *testing.T) {
	...
	second, err := detector.DetectLowerBound(context.Background())
	require.NoError(t, err)
	assert.Equal(t, map[protocol.LowerBoundType]int64{protocol.ProofBound: 100}, boundsByType(second))
}
