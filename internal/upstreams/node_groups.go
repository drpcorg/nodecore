...
func groupKeyOf(state *protocol.UpstreamState) GroupKey {
	...
	return GroupKey{ClientType: clientType, MethodsHash: methodsHash(state.UpstreamMethods)}
}
...
func newGroupState(key GroupKey) *groupState {
...
func (b *BaseChainSupervisor) GetNodeGroupStates() map[string]ChainSupervisorState {
	...
	b.groups.Range(func(_ GroupKey, g *groupState) bool {
		states[g.id] = g.state.Load()
		return true
	})
	return states
}
...
func (b *BaseChainSupervisor) assignGroup(id string, state *protocol.UpstreamState) {
	...
	oldKey, had := b.upstreamGroup[id]
	if had && oldKey == newKey {
		// same group, a member's state changed
		if g, ok := b.groups.Load(newKey); ok {
			b.recomputeGroup(g)
		}
		return
	}
	if had {
		b.leaveGroup(id, oldKey)
	}
	g, _ := b.groups.LoadOrStoreLazy(newKey, func() *groupState { return newGroupState(newKey) })
	g.members.Add(id)
	b.upstreamGroup[id] = newKey
...
func (b *BaseChainSupervisor) removeFromGroup(id string) {
	if key, ok := b.upstreamGroup[id]; ok {
