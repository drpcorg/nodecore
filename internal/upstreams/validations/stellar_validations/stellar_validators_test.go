...
func networkBody(passphrase string) []byte {
	return []byte(fmt.Sprintf(`{"passphrase":"%s","protocolVersion":27}`, passphrase))
}
...
func TestStellarHealthAvailable(t *testing.T) {
	...
	v := stellar_validations.NewStellarHealthValidator("id", conn, chains.GetChain("stellar"), time.Second)
